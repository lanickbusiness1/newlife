...
function ScoreBreakdown({ action }: { action: CareerNextAction }) {
  ...
  return (
    <details className="pathway-score-details">
...
function PathwayCard({ action }: { action: CareerNextAction }) {
  ...
  return (
    <article className={`candidate-card pathway-card status-${action.eligibility.status.toLowerCase()}`}>
      <div className="pathway-card-head">
        ...
        <span className="pathway-status">{STATUS_LABELS[action.eligibility.status]}</span>
      </div>
      ...
      {action.whyThisNext.length > 0 && (
        <div className="pathway-reasons">
          {action.whyThisNext.map((reason) => <span key={reason}>{reason}</span>)}
        </div>
      )}

      {missing.length > 0 && (
        <div className="pathway-missing">
      ...
      {action.eligibility.reviewRuleIds.length > 0 && (
        <p className="pathway-review-copy">Des critères du programme ou de la vacance doivent être vérifiés sur la source officielle avant toute décision.</p>
      )}

      <ScoreBreakdown action={action} />
      <a className="candidate-button secondary pathway-source" href={action.opportunity.sourceUrl} target="_blank" rel="noreferrer">
        Source officielle
      </a>
    </article>
  );
...
export function CareerPathwayPanel() {
  ...
  return (
    <section className="candidate-shell candidate-section pathway-section" aria-labelledby="career-pathway-title">
      <div className="candidate-card pathway-control">
        ...
        <form className="pathway-form" onSubmit={submit}>
          ...
          <div className="pathway-form-row">
            <input
              id="career-goal"
              value={goal}
        ...
        <p className="pathway-disclaimer">Aucune candidature n’est envoyée automatiquement. Les données inconnues restent inconnues et déclenchent une vérification.</p>
        {error && <div className="candidate-alert error" role="alert">{error}</div>}
      </div>

      {result && (
        <div className="pathway-results" aria-live="polite">
          <div className="pathway-results-head">
            <div>
              <span className="candidate-kicker">Objectif · {result.goal.title}</span>
              <h2>Prochaines étapes recommandées</h2>
            </div>
            <span className="pathway-count">{result.actions.length} opportunités analysées</span>
          </div>

          {missingFacts.length > 0 && (
            <div className="candidate-alert pathway-data-alert">
