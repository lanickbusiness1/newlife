...
export const POST = createCandidateRoute(async (request) => {
  ...
  const verifiedMetrics = body.verifiedMetrics.map((raw) => {
    ...
    return { value: requiredString(metric.value, 'metric value', 300), sourceRef: requiredString(metric.sourceRef, 'metric sourceRef', 200) };
  });
  const { auth, service } = await createCandidateRuntime(request);
  return Response.json(await service.rewrite(auth.candidate.id, sourceRef, sourceStatement, verifiedMetrics));
});
