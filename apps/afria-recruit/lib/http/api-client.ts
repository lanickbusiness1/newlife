...
export type DiagnosticResponse = {
  decisionId: string;
  diagnostic: { findings: DiagnosticFinding[] };
};
...
function accessToken() {
  ...
  return window.localStorage.getItem('afria_recruit_access_token');
}

async function requestJson<T>(path: string, init: RequestInit = {}): Promise<T> {
  const token = accessToken();
  if (!token) throw new CandidateApiError(401, 'Connexion requise');
...
export const candidateApi = {
  ...
  rewrite: (sourceRef: string, sourceStatement: string) => requestJson<{ decisionId: string; rewrite: { text: string; usedMetrics: string[] } }>('/api/candidate/rewrite', { method: 'POST', body: JSON.stringify({ sourceRef, sourceStatement, verifiedMetrics: [] }) }),
