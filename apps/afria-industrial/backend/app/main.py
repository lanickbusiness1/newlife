...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    telemetry_service = TelemetryService(conn); app.state.telemetry_service = telemetry_service
    @app.get('/health/live')
    ...
    app.include_router(build_telemetry_router(telemetry_service)); evidence_service = EvidenceService(conn)
    app.include_router(build_evidence_router(evidence_service)); app.include_router(build_readiness_router(ReadinessService(conn,evidence_service)))
    return app
app = create_app()
