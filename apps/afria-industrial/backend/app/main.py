...
def create_app(settings:Settings|None=None)->FastAPI:
    ...
    app.add_middleware(CORSMiddleware,allow_origins=origins,allow_credentials=False,allow_methods=['GET','POST','OPTIONS'],allow_headers=['X-API-Key','Content-Type'])
    conn=connect_sqlite(cfg.database_path); initialize_schema(conn); app.state.conn=conn; app.state.settings=cfg
    limiter=FixedWindowRateLimiter(cfg.mutation_rate_limit,cfg.mutation_rate_window_seconds); app.state.rate_limiter=limiter
    mutation_dependency=require_role(cfg,'engineer',limiter=limiter); telemetry_service=TelemetryService(conn); app.state.telemetry_service=telemetry_service; evidence_service=EvidenceService(conn)
    @app.get('/health/live')
    ...
    def ready()->dict[str,str]:
        ...
        return {'status':'ready'}
    @app.get('/system/mode')
    def system_mode()->dict[str,str]: return {'mode':cfg.system_mode,'source':'local'}
